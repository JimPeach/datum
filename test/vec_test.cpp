...
TEST_CASE("vec_construction", "[vec]") {
    SECTION("default_vec_is_empty") {
        dtm::vec<construction_test_type> vec;
        CHECK(vec.size() == 0);
    ...
    SECTION("N_default_construction_calls") {
        ...
        dtm::vec<construction_test_type> vec(5);
        CHECK(vec.size() == 5);
    ...
    SECTION("N_copy_construction_calls") {
        ...
        dtm::vec<construction_test_type> vec(5, construction_test);
        CHECK(vec.size() == 5);
        CHECK(vec.capacity() == 5);
        CHECK(!vec.empty());
    ...
    SECTION("copy_construction_calls") {
        dtm::vec<construction_test_type> v1(2);
        v1.reserve(5);

        construction_test_type::reset();
        dtm::vec<construction_test_type> v2(v1);        
        CHECK(construction_test_type::num_default_constructions == 0);
    ...
    SECTION("move_construction_calls") {
        ...
        construction_test_type::reset();
        dtm::vec<construction_test_type> v2(std::move(v1));       
        CHECK(construction_test_type::num_default_constructions == 0);
        CHECK(construction_test_type::num_copy_constructions == 0);
        CHECK(construction_test_type::num_move_constructions == 0);
        CHECK(construction_test_type::num_destructions == 0);
    ...
    SECTION("from_input_iterator") {
        ...
        dtm::vec<int> v(std::begin(base_vec), std::end(base_vec));
        CHECK(v.size() == 5);
        CHECK(v[0] == 0);
        CHECK(v[1] == 1);
        CHECK(v[2] == 2);
...
TEST_CASE("vec_reserve", "[vec]")
{
    ...
    SECTION("from_non_empty") {
        dtm::vec<construction_test_type> vec(1);
...
TEST_CASE("vec_resize", "[vec]")
{
    SECTION("shrink") {
        dtm::vec<construction_test_type> vec(10);
        REQUIRE(vec.size() == 10);
...
TEST_CASE("vec_shrink_to_fit", "[vec]") {
    dtm::vec<construction_test_type> vec(10);
    vec.resize(5);
    REQUIRE(vec.capacity() == 10);
...
TEST_CASE("vec_push_back", "[vec]") {
    SECTION("many_push_backs") {
        ...
        CHECK(!v.empty());
        CHECK(v.capacity() < 2*N + dtm::vec<int>::minimum_growth_size);
        REQUIRE(v.size() == N);
        for (int i = 0; i < N; i++)
            CHECK(v[i] == i);
    }
    ...
    SECTION("copy") {
        dtm::vec<construction_test_type> v;
        v.reserve(5);
...
TEST_CASE("vec_pop_back", "[vec]")
{
    ...
    SECTION("calls_destructor") {
        dtm::vec<construction_test_type> v(1);
...
TEST_CASE("vec_forward_iterator", "[vec]")
{
    SECTION("empty") {
        dtm::vec<int> v;
